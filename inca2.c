...
I *ma(n){R(I*)malloc(n*sizeof(intptr_t));}
mv(d,s,n)I *d,*s;{DO(n,d[i]=s[i]);}
tr(r,d)I *d;{I z=1;DO(r,z=z*d[i]);R z;}
ARC ga(t,r,d)I *d;{I n;
    ARC z=(ARC)ma((sizeof(*z)/sizeof(I))+(n=tr(r,d)));AT(z)=t,AR(z)=r,AN(z)=n,mv(AD(z),d,r);
    R z;}
V1(iota){
...
V1(sha){
    ARC z=ga(INT,1,&AR(w));mv(AV(z),AD(w),AR(w));R z;}
V1(id){R w;}
V1(size){
...
nl(){printf("\n");}
pr(w)ARC w;{
    I r=AR(w),*d=AD(w),n=AN(w);DO(r,pi(d[i]));nl();
    if(AT(w)==BOX)DO(n,printf("< ");pr(AV(w)[i]))
    else DO(n,pi(AV(w)[i]));nl();}
...
noun(c){ARC z;if(c<'0'||c>'9')R 0;z=ga(0,0,0);*AV(z)=c-'0';R (I)z;}
